...
function App() {
  ...
  useEffect(() => {
    ...
    // HERSTELD: Veilige manier om thema toe te passen, met fallback naar de standaard.
  ...
  return (
    <div className="pt-8 min-h-screen font-sans transition-colors duration-500">
      <header className="bg-gray-800 p-4 shadow-md sticky top-0 z-20">
        <div className="container mx-auto flex justify-between items-center">
          ...
          <nav className="hidden md:flex items-center space-x-4">
            <NavButton to="/" label="Zoeken" /><NavButton to="/lists" label="Mijn Lijsten" /><NavButton to="/recommendations" label="Aanbevelingen" /><NavButton to="/settings" label="Instellingen" />
      ...
      <main className="container mx-auto p-4 md:p-8">
        <Routes>
          <Route path="/" element={<SearchPage />} /><Route path="/lists" element={<ListsPage />} /><Route path="/recommendations" element={<RecommendationsPage />} /><Route path="/settings" element={<SettingsPage />} />
        </Routes>
      </main>
    </div>
  );
