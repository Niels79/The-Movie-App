...
function App() {
  ...
  const TopNavButtonMobile = ({ to, label, icon }: { to: string, label: string, icon: React.ReactNode }) => {
  ...
  return (
    // =======================================================================
    // DE AANPASSING ZIT HIER: `pt-8` is toegevoegd voor extra ruimte bovenaan.
    // =======================================================================
    <div className={`pt-8 min-h-screen font-sans transition-colors duration-500`}>
      ...
      {notification && (
        <div className="fixed top-20 right-5 bg-green-500 text-white py-2 px-5 rounded-lg shadow-lg z-30 animate-pulse">
            {notification}
        </div>
      )}
